...
#+begin_src js :cmd "NODE_PATH=./node_modules node"
  ...
  const drawMazeTagged = () => {
      ...
      tagMaze(ctx)
      canvas.createPNGStream().pipe(
          fs.createWriteStream('images/tagged.png')
      )
  }
...
#+begin_src dot :file images/maze-graph.png :cmdline -Kdot -Tpng :exports both
  digraph {
    Вход
    Выход
    Азел  [color="green"]
...
#+begin_src dot :file images/maze-graph-path.png :cmdline -Kdot -Tpng :exports both
  digraph {
    Вход
    Выход
    Азел  [color="green"]
